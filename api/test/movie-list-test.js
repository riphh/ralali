...
const testCase = {
 ...
 "negative" : {
	...
	"invalidkeySearch" : "As a User, I should got error 200 when I send request with invalid Search Key",
	"noApiKey" : "As a User, I should got error 401 when I send request with no API Key",
	"noApiKeyNoSearch" : "As a User, I should got error 401 when I send request with no API Key and no Key Search"
	}
 };
...
describe(`OMDB Movie List`, () => {
 ...
 const invalidkeySearch = 'afasdfasdfasdf';
 ...
 it(`@get ${testCase.negative.invalidkeySearch}`, async() => {
  const response = await page.getMovieList(apiKey, invalidkeySearch);
  assert(response.status).to.equal(200);
  assert(response.body.Response).to.equal('False');
 ...
 it(`@get ${testCase.negative.noApiKeyNoSearch}`, async() => {
  const response = await page.getMovieList('','');
  assert(response.status).to.equal(401);
  assert(response.body.Response).to.equal('False');
  assert(response.body.Error).to.equal('No API key provided.');
 }); 
